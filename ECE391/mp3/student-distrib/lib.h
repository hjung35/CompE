...
#define NUM_COLS    80
#define NUM_ROWS    25
#define LINE_MEM_LEN NUM_COLS * 2
#define ATTRIB      0x7
...
/* IO wait.
 * Apparently you can make use of another IO on an unused port
 * to make the CPU wait in between output instructions (specifically for the PIC).
 * Port 0x00B0 appears to be free according to https://wiki.osdev.org/I/O_Ports
 */
#define io_wait()                           \
do {                                        \
    asm volatile ("outb %%al, $0xB0"        \
                    :                       \
                    : "a"(0)                \
                    );                      \
} while (0)                                 
